...
- `polity21` - The Polity2 democracy score
- `ethfrac` - Ethno-linguistic fractionalization index
- `relfrac` -  Religious fractionalization index
- `anocl` - An indicator for "anocracy" (regime interruption)
- `deml`  - A dichotomous measure of democracy

## Replicate Model 1 from Fearon and Laitin (20%)

Run the first model (Model 1) from Table 1 of the Fearon and Laitin article. Use `summary()` to view the results. Interpret the coeffiient on the Oil exporter variable. How much does being an oil exporter increase the risk of conflict onset? 
...
Interpret the results. Which predictor has largest effect on the probability of conflict onset? Which one has the least? 
